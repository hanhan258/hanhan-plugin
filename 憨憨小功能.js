...
export class jiami extends plugin {
  ...
  async ping (e) {
    ...
    let ipAddress = e.msg.trim().match(/^#?ping\s?(.+)/)[1]
    if (!ipAddress) {
    ...
    if (!regex.test(ipAddress)) {
      ...
      try {
        ipAddress = await lookup(ipAddress)
      } catch (err) {
    ...
    // 获取ping结果
    let pingRes = await new Promise((resolve, reject) => {
      exec(pingCommand, (error, stdout, stderr) => {
        if (error) {
          logger.error(`exec执行出错: ${error}`)
          reject(error)
        } else {
          resolve(stdout)
        }
    ...
    // 替换ip地址
    pingRes = pingRes.replace(new RegExp(ipAddress, 'g'), '=͟͟͞͞ʕ•̫͡•ʔ=͟͟͞͞ʕ•̫͡•ʔ=͟͟͞͞ʕ•̫͡•ʔ')
    // 获取ip信息
    let ipInfoRes = await fetch(`https://ipinfo.io/${ipAddress}?token=${token}`)
